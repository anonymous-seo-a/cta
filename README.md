...
Phase 2: 自サイト+競合スクレイプ → Claude APIリライト案生成  ✅ 完了
    ↓
Phase 2.5: Gutenbergマークアップ生成 → Spreadsheet出力       ✅ 完了
    ↓
人間がSpreadsheetのマークアップをWPエディタに貼り付けて確認
    ↓
Phase 3: 承認済みマークアップを自動入稿                      ✅ 完了
```
...
### 主要関数（seo_rewrite_markup.gs）
| 関数 | 用途 | 実行 |
|---|---|---|
| `runRewritePhase25` | WPコンテンツ+分析結果→Claude APIでGutenbergマークアップ生成 | 手動 |
| `applyApprovedMarkup` | 承認済みマークアップをWordPress反映 | 手動 |
