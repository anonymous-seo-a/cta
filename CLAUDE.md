...
clasp/                    ← ★ GASファイルの正（clasp push でGASに反映）
  ...
  main.js                 - CVR診断（機能1）+ 共通ユーティリティ
  cta_insertion.js        - CTA自動挿入（機能2）
  seo_rewrite.js          - SEOリライト Step 1-2（機能3前半）
  seo_rewrite_markup.js   - SEOリライト Step 3-4（機能3後半）
  annotation_master.js    - 注釈マスターデータ管理
  gsc_master.js           - GSCデータ一元管理（日次バッチ）

gas/                      ← アーカイブ（clasp導入前の.gsコピー。編集しない）
...
- **ビルドステップなし**: .js/.gsファイルがそのまま実行される（claspは.jsを.gsとしてデプロイ）
