...
### カードローン記事フルフロー検証 + 注釈処理の構造的問題修正

**テスト記事:** 14618（今すぐお金が必要 借りる以外）

**Step 4テスト:** 成功。WP REST APIでの入稿＋Indexing API通知の動作確認完了。

**カテゴリ均等選定の実装:**
- `getRewriteCandidates`: ラウンドロビンで4カテゴリ均等選定
- `runRewritePhase1_Cardloan` 等のカテゴリ別ラッパー追加
- `REWRITE_CONFIG.TARGET_CATEGORIES` 追加

**Indexing API通知の実装:**
- `applyApprovedFulltext` 内でWP入稿成功後に `notifyGoogleUrlUpdated()` 呼び出し
- `appsscript.json` にindexingスコープ追加
- GCPでWeb Search Indexing APIの有効化が必要

**注釈処理の修正（5回のイテレーション）:**

1. **修正A**: 再利用/CTA含有セクションの削除禁止（維持に強制変更）
2. **修正B**: Claude APIに注釈挿入を禁止（ポスト処理に一元化）
3. **修正C**: 複数商材言及時の注釈自動挿入スキップ → 後に番号参照方式に変更
4. **修正D**: 見出しのid属性保持ルール追加
5. **修正E**: ボックス内ul/li禁止 + wp:htmlラッパー必須化

**再利用ブロック保護の調整:**
- 最初: 全面維持（書き換え・見出し変更も禁止）→ 過剰
- 最終: 削除・統合のみ禁止、書き換え・見出し変更は許可

**注釈処理の全面改修:**
- extractAnnotationsToPlaceholders: 出典リンクと記号定義のみ退避、商材注釈は除去
- postProcessAnnotations: 全トリガーKW出現箇所に新規挿入、番号参照方式対応

**残存する未解決問題（次セッションで対応）:**
- 注釈処理がセクション単位で実行されるため、「維持」セクション（再利用ブロック含有）には注釈処理が走らない
- 結果、各社紹介セクションの注釈が元記事のまま（位置がおかしい・形式が不統一）
- 目次ナビ内のテキストにも注釈が入らない
- **対策方針確定済み**: セクション単位のpostProcessを廃止し、全文結合後に1回だけ注釈処理を実行する

**コミット履歴（Session 5）:**
- 743ba06 feat: 注釈挿入を全KW出現箇所に適用 + 番号参照方式
- cddb9f1 feat: 注釈処理を全面改修 — 出典保護＋商材注釈を全KW出現箇所に新規挿入
- c08e3c9 fix: 再利用ブロック含有セクションの見出し変更・書き換えを許可
- 2af6b61 fix: 再利用ブロック含有セクションを強制維持 + wp:htmlラッパー修正
- ff047cb fix: 注釈処理の構造的欠陥5件を修正
- 16524bf feat: Step4入稿後にGoogle Indexing APIで更新通知
- a7250ab feat: カテゴリ均等選定（ラウンドロビン）+ カテゴリ別Phase1ラッパー
